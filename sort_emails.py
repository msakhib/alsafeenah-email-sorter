...
import json, logging, os, sys, urllib.parse, urllib.request
...
# ── Constants ─────────────────────────────────────────────────────────────────
ACCOUNT_ID     = "2108081000000008002"
...
def fetch_inbox_emails(token):
    url = (
        f"{ZOHO_API_BASE}/accounts/{ACCOUNT_ID}/messages/view"
        f"?folderId={INBOX_ID}&limit=100&sortby=date&sortorder=false&status=all"
    )
    return zoho_get(url, token).get("data", [])


def move_email_batch(token, message_ids, destination_folder_id):
...
Spam:
  ...
  - Cold IT/digital: website design, SEO, PPC, social media, app development, IT support, Microsoft 365 renewal (unknown vendors)
  - Cold financial/legal: business loans, insurance, unsolicited accounting/tax/trademark
  - Tender platform spam: UAE Business Gate, UAE Tenders Platform, GCC Chambers, ADNOC mass-mailer
  - Chinese factory mass-marketing with no prior relationship
  - Scams, phishing, fake "new order" from unknown parties
  - Mass manpower recruitment with no specific role

Payment Remittance:
  - Subject has: remittance, payment advice, payment confirmation, remittance advice
...
Job Applications:
  - Subject/body: CV, resume, vacancy, career, applying for, looking for job, seeking employment
  - Manpower/recruitment agency referencing a specific role
...
Suppliers:
  ...
  - NOT curtains/software/SEO companies (those are Spam)

Operational:
  - Sender domain: zoho.com, microsoft.com, yellowpages-uae.net
  - System alerts, login notifications, logistics updates, YellowPages, internal @alsafeenah.ae emails
  - Vendor portal gate pass / access notifications (non-payment, non-order)

Business Critical (DEFAULT for genuine client/project emails):
  - RFQ, enquiry, inquiry, request for quotation, please quote, kindly quote, tender
  - Machining keywords: gear, roller, shaft, flange, bushing, bearing, coupling, pipe spool, fabrication,
    CNC, lathe, milling, welding, fitting, rebar coupler, bollard, hydraulic, pressure die, sprocket
  - Active client threads, POs, delivery notes, MTC requests, invoices related to a job
...
def classify_emails(emails, api_key):
    """Send emails to Claude and get folder assignments back as JSON."""
    if not emails:
        return []
...
def main():
    ...
    try:
        emails = fetch_inbox_emails(token)
    ...
    # 4. Group by folder and move (batches of 10)
    ...
    for folder_name, ids in folder_groups.items():
        ...
        for i in range(0, len(ids), 10):
            batch = ids[i:i+10]
            try:
                move_email_batch(token, batch, folder_id)
